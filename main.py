...
try:
    ...
    while finishProgram is False:
        ...
        if counter > 0:
            optionContinue = int(input(
                'Do you want to do a new search or Update? Yes[1] No[2]:  '
                ))
            if optionContinue == 2:
        ...
        if optionSelect == 1:
            ...
            if optionUpdate == 1:
                newUser = True
                searchUser = str(input(
                    "Insert the name that you want to insert Score:"
                    ))
                scoreUpdate = int(input("Insert the Score:"))
                myPythonFunction.updateUserPoints(
                    newUser, searchUser, scoreUpdate
                    )
            elif optionUpdate == 2:
                newUser = False
                searchUser = str(input(
                    "Insert the name that you want to update the Score:"
                    ))
